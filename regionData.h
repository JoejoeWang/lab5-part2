...
class regionData {
  public:
    ...
    regionData(const string inS, const int pop) : regionName(""), population(0) { stateS.insert(inS); }

    regionData(const string inN, const string inS) : regionName(inN), population(0) { stateS.insert(inS); }
    ...
    int StateSsize() const { return stateS.size(); }
